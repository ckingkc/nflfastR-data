...
base_kicks <- load_pbp(1999:2020) %>% 
  ...
  summarise(
    ...
    fg_made_50_ = sum(as.numeric(field_goal_result == "made") * (kick_distance >=50), na.rm = TRUE),
    ...
    fg_missed_50_ = sum(as.numeric(field_goal_result == "missed") * (kick_distance >=50), na.rm = TRUE),
